...
.\tools\curl\bin\curl.exe --insecure -E ./certificates/sysop.crt.pem --key ./certificates/sysop.key.pem -X POST "https://arrowhead2.tmit.bme.hu:18443/serviceregistry/mgmt/systems" -H "accept: application/json" -H "Content-Type: application/json" -d "{ \"address\": \"localhost\", \"authenticationInfo\": \"%PUBLICKEY%\", \"port\": 8080, \"systemName\": \"%CONSUMERNAME%\"}" | .\tools\jq\jq.exe ".id" > consumerid.txt

ECHO AUTHORIZING PROVIDER-CONSUMER COMMUNICATION...
.\tools\curl\bin\curl.exe --insecure -E ./certificates/sysop.crt.pem --key ./certificates/sysop.key.pem -X GET "https://arrowhead2.tmit.bme.hu:18443/serviceregistry/mgmt/systems?direction=ASC&item_per_page=0&page=0&sort_field=id" -H "accept: */*" | .\tools\jq\jq.exe ".data[] | select(.systemName == \"%PROVIDERNAME%\") | .id" > providerid.txt
.\tools\curl\bin\curl.exe --insecure -E ./certificates/sysop.crt.pem --key ./certificates/sysop.key.pem -X GET "https://arrowhead2.tmit.bme.hu:18443/serviceregistry/mgmt/services?direction=ASC&item_per_page=0&page=0&sort_field=id" -H "accept: application/json" | .\tools\jq\jq.exe ".data[] | select(.serviceDefinition == \"temperature\") | .id" > servicedefinitionid.txt
.\tools\curl\bin\curl.exe --insecure -E ./certificates/sysop.crt.pem --key ./certificates/sysop.key.pem -X GET "https://arrowhead2.tmit.bme.hu:18443/serviceregistry/mgmt/servicedef/temperature?direction=ASC&sort_field=id" -H "accept: */*" | .\tools\jq\jq.exe ".data[] | select(.interfaces[].interfaceName == \"HTTP-INSECURE-SENML\") | .interfaces[].id" > interfaceid.txt
