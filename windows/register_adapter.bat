...
.\tools\curl\bin\curl.exe --insecure -E ./certificates/sysop.crt.pem --key ./certificates/sysop.key.pem -X POST "https://127.0.0.1:8443/serviceregistry/mgmt/systems" -H "accept: application/json" -H "Content-Type: application/json" -d "{ \"address\": \"localhost\", \"authenticationInfo\": \"%PUBLICKEY%\", \"port\": 8080, \"systemName\": \"%CONSUMERNAME%\"}" | .\tools\jq\jq.exe ".id" > providerid.txt

ECHO AUTHORIZING PROVIDER-CONSUMER COMMUNICATION...
.\tools\curl\bin\curl.exe --insecure -E ./certificates/sysop.crt.pem --key ./certificates/sysop.key.pem -X GET "https://127.0.0.1:8443/serviceregistry/mgmt/systems?direction=ASC&item_per_page=0&page=0&sort_field=id" -H "accept: */*" | .\tools\jq\jq.exe ".data[] | select(.systemName == \"%CONSUMERNAME%\") | .id" > consumerid.txt
